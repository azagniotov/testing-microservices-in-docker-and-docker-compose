...
Testing Microservices in Docker and Docker Compose with stubby4j, an HTTP stub server.

## Overview

The current repository contains the code which accompanies the tutorial [TBD Testing Microservices in Docker and Docker Compose] Medium blog post. The tutorial talks about local testing of microservice interactions in Docker Compose environment and stubbing some of these microservices in stubby4j, an HTTP stub server. The tutorial emphasizes the usage of stubby4j in order to demonstrate how it can help with testing of microservices.

Docker Compose is a tool for defining and running multi-container Docker applications. With Compose, you use a YAML file to configure your application’s services. Then, with a single command, you create and start all the services from your configuration.

stubby4j is an HTTP server for stubbing external systems in both Docker and non-containerized environments for integration, contract and behavior testing. stubby4j allows you to configure (i.e.: to stub) various failure/success scenarios with simple HTTP responses, as well as complex HTTP responses. See [stubby4j key features](https://github.com/azagniotov/stubby4j#key-features) for more information.

## Introduction

When developing and testing microservices, it is a good practice to focus on testing interactions of the service with its direct upstream/downstream dependencies.
...
In order to perform such tests, you would have to fake your indirect dependencies. Faking your dependencies can be easily achieved with a powerful library like [stubby4j](https://github.com/azagniotov/stubby4j), an HTTP stub server.

There are a number of use cases where you'd want to use HTTP stub server in your development/QA environment. If you are a `Software Engineer`/`Test Engneer`/`QA`, then this should hit close to home for you.

As an example, some of these use cases are outlined below (this is by no means an exhaustive list). `stubby4j` can help you when you want to:
...
We use docker compose to spin up two local Docker containers in an isolated environment using the definitions in the local [docker-compose.yaml](docker-compose.yaml):
...
`stubby4j` stubs defined in the following YAML config [stubby4j-yaml-config/stubs.yaml](stubby4j-yaml-config/stubs.yaml). The `Account` service specific stub are defined in [stubby4j-yaml-config/include-account-service-stubs.yaml](stubby4j-yaml-config/include-account-service-stubs.yaml), while the `Reservation` service specific stubs are defined in [stubby4j-yaml-config/include-reservation-service-stubs.yaml](stubby4j-yaml-config/include-reservation-service-stubs.yaml)
...
To understand in more depth the `stubby4j` configuration options beyond the current tutorial, see the [Endpoint configuration HOWTO](https://github.com/azagniotov/stubby4j#endpoint-configuration-howto) for more information.
...
The command builds `Flight Booking` Docker image using the local [Dockerfile](Dockerfile) (this can take a few minutes, please be patient) and fetches pre-built `stubby4j` Docker image from [https://hub.docker.com/r/azagniotov/stubby4j](https://hub.docker.com/r/azagniotov/stubby4j)
...
* `Account` service stubbed endpoint `/api/v1/accounts/1` always responds with the HTTP status `200 OK` and the same HTTP JSON response defined in [stubby4j-yaml-config/json/account.service.expected.success.response.json](stubby4j-yaml-config/json/account.service.expected.success.response.json)
* `Reservation` service stubbed endpoint `/api/v1/reservations/1` responds with different HTTP responses on each invocation of `http://localhost:8080/api/v1/flights/1` . There are a total of five different HTTP responses that `stubby4j` loops over when rendering responses for `/api/v1/reservations/1`: `200 OK`, `400 Bad Request`, `403 Forbidden` and `500 Internal Server Error` with different JSON response bodies defined under [stubby4j-yaml-config/json](stubby4j-yaml-config/json).
...
In this tutorial, we've learned how to test microservices running in Docker environment by leveraging the Docker Compose and `stubby4j`, an HTTP stub server.
